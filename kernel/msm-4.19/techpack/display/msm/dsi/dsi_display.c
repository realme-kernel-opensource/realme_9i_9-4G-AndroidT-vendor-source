...
#ifdef OPLUS_BUG_STABILITY
	/* After fisrt frame commit, delay backlight up */
	if (bl_lvl != 0 && panel->bl_config.bl_level == 0) {
		if (panel->oplus_panel_power.panel_on_mdelay_bl)
			oplus_mdelay(panel->oplus_panel_power.panel_on_mdelay_bl);
	}
#endif
...
int dsi_display_disable(struct dsi_display *display)
{
	...
	if (!display->poms_pending) {
		...
		if (!strcmp(display->panel->name,"ili9883c 90hz boe hd video mode dsi panel")
			   && ((mode == MSM_BOOT_MODE__FACTORY) || (mode == MSM_BOOT_MODE__RF)
			   || (mode == MSM_BOOT_MODE__WLAN))) {
			pr_err("[TP] in ftm mode, set tp_ftm_extra\n");
			tp_ftm_extra();
		}
		#endif /* OPLUS_FEATURE_TP_BASIC*/
		#endif
