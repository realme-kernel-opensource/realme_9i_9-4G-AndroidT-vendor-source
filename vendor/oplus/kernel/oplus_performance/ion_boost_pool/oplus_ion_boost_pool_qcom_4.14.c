...
static char boost_pool_thread_comm[TASK_COMM_LEN];
...
static bool is_pools_fill_thread(void)
{
	return !strncmp(current->comm, boost_pool_thread_comm, TASK_COMM_LEN);
}
		      ...
		      int nr_to_scan)
{
	...
	if (gr_pool_is_full(boost_pool)) {
		...
		if ((other_free + other_file > totalram_pages / 10) ||
			(boost_pool_nr_pages(boost_pool) <= ori_low_pages))
			return 0;
		if (nr_to_scan)
			nr_to_scan = min(nr_to_scan, 1024);
		else
			nr_to_scan = 1024;
		if ((boost_pool_nr_pages(boost_pool) - nr_to_scan) < boost_pool->low)
			boost_pool->low = boost_pool->high = boost_pool_nr_pages(boost_pool) - nr_to_scan;
	}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	boost_pool->low = boost_pool->high = gr_pool_should_protect = nr_pages;
					 ...
					 char *name)
{
	...
	tsk = kthread_run(boost_pool_kworkthread, boost_pool,
			  "bp_%s", name);
	snprintf(boost_pool_thread_comm, TASK_COMM_LEN, "bp_%s", name);
