...
static int msm_compr_read_buffer(struct msm_compr_audio *prtd)
{
	...
	if (atomic_read(&prtd->error)) {
		pr_err_ratelimited("%s Got RESET EVENTS notification", __func__);
		return -ENETRESET;
	}
	#endif
					...
					char __user *buf, size_t count)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	if (msm_compr_read_buffer(prtd) == -ENETRESET) {
		pr_err_ratelimited("%s adsp is down, read failed", __func__);
		atomic_set(&prtd->error, 1);
		spin_unlock_irqrestore(&prtd->lock, flags);
		return -ENETRESET;
	}
